...
- **D-3 (Race-Erkennung + Round):** Die Evaluation-λ klärt per OpenF1 `/sessions` (2 Calls, Zod-validiert), ob `session_id` ein Rennen ist, und berechnet `round` mit derselben Logik wie Schedule-Sync (1-basierte Position unter den Race-Sessions der Saison) — so matcht der PK exakt den der Inference-λ.
