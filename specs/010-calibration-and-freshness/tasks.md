...
- [x] **T20** — realer `refresh_history`-Lauf für 2026 R1–R11 am 2026-07-29. **Nicht** als Überschreibung von `0.2.0`, sondern als neue Version `0.2.1` (siehe T21): 1838 → 2080 Zeilen, +242 (11 Runden × 22 Fahrer), Saisons `[2022, 2023, 2024, 2025, 2026]`. Datenprüfung: die Podien der Runden 8–11 reproduzieren exakt die unabhängig aus Jolpica gemessenen (`RUS VER ANT`, `LEC RUS HAM`, `ANT LEC VER`, `NOR VER ANT`); Fehlwertquoten liegen im Rahmen der Vorsaisons (Quali 1,2 % vs. 0,6–1,7 %, Practice erhöht durch mehr Sprint-Wochenenden).
- [x] **T21** _(nachgezogen)_ — Konstruktionsfehler behoben: `refresh_history.py` überschrieb standardmäßig die gelesene Version. Das hätte die unter `0.2.0` gespeicherten Vorhersagen der Runden 1–11 unreproduzierbar gemacht — derselbe Fehler, den die „never `latest/`"-Regel verhindern soll (Constitution IX). Neu: `--target-version` ist Pflicht, Gleichheit mit `--version` wird abgelehnt, `model.json` + `model_card.md` werden mitkopiert. `ACTIVE_MODEL_VERSION` → `0.2.1`, `MODEL_PROVENANCE` trennt Trainingsfenster von History-Reichweite, und der Offenlegungstext sagt für Archivrennen (0.2.0) und neue Rennen (0.2.1) jeweils das Zutreffende.
